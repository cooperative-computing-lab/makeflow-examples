...
def parse_bwa_options(parser):
	...
	group.add_option('--cores', dest='cores', help='Specifies the minimun amount of cores needed for each job, adds -t for each job')
	group.add_option('--disk', dest='disk', help='Specifies the minimum amount of disk space needed for each job')
	group.add_option('--category', dest='category', help='Specifies the category needed for each job')
	group.add_option('--memory', dest='memory', help='Specifies the minimim amount of memory needed for each job')
...
def write_makeflow(destination, configuration, num_reads_per_split, fastq_name, cat_name, bwa_args, algo):
	...
	try:
		...
		if (options.category) :
			makeflow.write("\nCATEGORY={0}".format('"' + options.category)+'"')
		if (options.cores) :
			makeflow.write("\nCORES={0}".format(options.cores))	
		if (options.memory) : 
                        makeflow.write("\nMEMORY={0} ".format(options.memory))
		if (options.disk) :
                        makeflow.write("\nDISK={0} ".format(options.disk)) 
	 ...
	 # Rule and Command for Indexing FASTA File
		...
		cores= ""
		if(options.cores and (options.cores > 1)):
			cores= " -t {0} ".format(options.cores);
		...
		for i in range(splits):
			...
			if algo=="backtrack":
				...
				makeflow.write("\n\t./bwa {0} {1} {2} {3} {4} {5} > {6} 2> {7}".format(algo, cores, args, ref, query1, query2, output, error))
		...
		output_blob=(options.fastq) + ".*.sam"
		output_blob_err=(options.fastq) + ".*.err"

		if num_reads_per_split > 0:
			makeflow.write("\n\n{0} : {1} {2}".format(output, cat_name,' '.join(results_to_cat)))			
			makeflow.write("\n\tLOCAL perl {0} {1} {2}".format(cat_name, output, output_blob))	

			makeflow.write("\n\n{0} : {1}".format(error, ' '.join(errors_to_cat)))
			makeflow.write("\n\tLOCAL cat {0} > {1}".format(output_blob_err , error))	
